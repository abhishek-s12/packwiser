...
use packwiser_compressor::{ZipCompressor, TarGzCompressor};
...
fn test_compression_format_performance_comparison() {
    ...
    zip_comp.compress(&files, &mut zip_out, Box::new(|_| {})).unwrap();
    ...
    targz_comp.compress(&files, &mut targz_out, Box::new(|_| {})).unwrap();
    ...
    println!("TAR.GZ Speed: {:?}, Size: {} bytes", targz_duration, targz_size);
